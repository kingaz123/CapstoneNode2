  ...
  try {
    ...
    if (isUserExists || isEmailExists) {
      return res.json(
        ResponseError(
          400,
          ["Địa chỉ email hoặc tên tài khoản đã được đăng ký trước đó."],
          "Địa chỉ email hoặc tên tài khoản đã được đăng ký trước đó."
        )
      );
    }
    ...
    return res.json(
      ResponseSuccess(201, { taiKhoan, hoTen, email, soDt }, "Tạo tài khoản thành công")
    );
  } catch (error) {
    console.error("Error in registration:", error);
    return res.status(500).json(ResponseError(500, {}, "Server Error: " + error.message));
  }
};
